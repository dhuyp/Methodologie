...
La configuration peut être récupérée avec l'outil \tool{secedit.exe} en utilisant la commande suivante: \tool*{secedit /export /CFG secedit.txt}.\\
On peut aussi récupérer la configuration en accèdant à la clé de registre correspondante:\\ %\tool{HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Netlogon\Parameters}\\
HKEY\_LOCAL\_MACHINE\textbackslash`SYSTEM\textbackslash`CurrentControlSet\textbackslash`Services\textbackslash`Netlogon\textbackslash`Parameters
...
Les configurations sur la politique des comptes utilisateurs sont accessibles avec l'outil  \tool{secedit.exe} en utilisant la commande suivante:\\ \tool*{secedit /export /areas USER_RIGHTS /cfg policies.txt}.\\
