...
\pshell{[System.Environment]::OSVersion}.\\
Pour les dernières mises à jour :\\
avec \tool{wmic} : \pshell{ wmic qfe list},\\
avec \textbf{Powershell} : \pshell{Get-WmiObject -Class "win32_quickfixengineering"}.
...
\end{itemize}
Les ruches de registre sont des ensembles de clés et de valeurs. Ces ruches sont associés à un ensemble de fichiers. La liste des ruches et leurs fichiers correspondant est récupérable avec \tool{Reg} et  la commande suivante:\\
\pshell{reg query HKLM\System\CurrentControlSet\Control\Hivelist}\\

\begin{FlagConsole}{Utilisation de l'outil Reg }
\pshell{reg export <Nom  de la clé> <Fichier de sortie> [/y]}\\\\
On exporte la clé de registre dans un fichier de sortie.\\
/y : Ecrase un fichier existant
\tcblower
\pshell{reg query <KeyName> [{/v <ValueName> | /ve}] [/s]}\\\\
...
La politique de journalistaion ou \textbf{legacy audit policy}, elle est récupérable avec l'outil \tool{secedit.exe} avec la commande suivante:\\
...
Le SCT contient plusieurs sous-outils. Pour l'extraction des \textbf{Group Policy Object (GPO)} nous utilisons \tool{LGPO.exe}. Pour l'analyse de la configuration nous utilisons le \tool{PolicyAnalyze}. Il permet de comparer un ensemble de GPO avec les politiques locales actuelles et avec les registres locaux, en mettant en évidence les différences.

% extraction de la configuration avec l'outil LGPO

\begin{LongFlagConsole}{Analyse d'une configuration Windows 7}
\tcbsubtitle{Extraction de la configuration}
\pshell{LGPO.exe /b C:\output_dir}\\\\
Les GPOs sont extraitent avec \tool{LGPO.exe} dans le répertoire passé en paramètre. Le chemin du répertoire doit être absolue. L'outil va créer un répertoire au nom aléatoire contenant les GPOs.

\tcbsubtitle{Importation de la configuration}
Une fois la configuration extraite, il faut l'importer dans l'outil \tool{PolicyAnalyzer}

\begin{center}
	\includegraphics[width=.9\textwidth]{img/conf_windows/import_gpo.PNG}
	\label{fig:import_gpo}
	\captionof{figure}{Import des GPOs}
\end{center}

On ajoute le répertoire contenant les GPOs, puis on fait \textbf{Import} qui permet de créer le fichier \textbf{POLICYRULES}.
\begin{center}
	\includegraphics[width=.9\textwidth]{img/conf_windows/add_files_import_gpo.PNG}
	\label{ref:add_files_import_gpo}
	\captionof{figure}{Ajout des GPOs}
\end{center}

Si on a bien importé le fichier \textbf{POLICYRULES} dans le dossier \textbf{SamplePolicyRules}, il apparaît maintenant dans l'outil \tool{PolicyAnalyzer}.
\begin{center}
	\includegraphics[width=.9\textwidth]{img/conf_windows/my_gpos_added.PNG}
	\label{ref:my_gpos_added}
	\captionof{figure}{Nouvelles GPOs importées}
\end{center}

\tcbsubtitle{Analyse de la configuration}
...
\begin{center}
	\includegraphics[width=.9\textwidth]{img/conf_windows/compare_view_gpos.PNG}
	\label{ref:compare_view_gpos}
	\captionof{figure}{Vue comparative des GPOs}
\end{center}

\end{LongFlagConsole}

% petite phrase pour expliquer comment on interprète les résultats (pour les cas non traitée avec les recommendations fournis par ex)
