...
Une \gls{CSRF} peut se contre en utilisant:\begin{itemize}
...
\begin{Define}{Vulnérabilités!SID}
Une session est identifiée, côté serveur, par un \gls{SID}. Le SID est une information partagé entre le navigateur et le serveur et permet à un navigateur de se faire reconnaitre auprès du serveur. Il peut prendre différente forme: cookie, données POST, dans l'URL... La meilleure implémentation consiste cependant à transmettre cette donnée dans un cookie de session. Du fait de l'importance du \gls{SID}, de nombreuses protections doivent être mis en place contre son vol ou son usage par un tiers.
\end{Define}

\subsubsection{Injection de SID}
\subsubsection{Fixation de SID}
\subsubsection{Vol de session}
%utilisation de cookie secure en plus des notes initailes
...
L'attaque de moindre complexité pour AES consiste en une attaque par brute-force, la difficulté de l'attaque est donc fonction de la taille de la clef avec une complexité cryptographique de 128 bits pour AES-128 et 256 bits pour AES-256. La complexité cryptographique de DES est de 80 pour 2DES (ou \textit{2TDEA}) soit une clef de 112 bits et 112 pour 3DES soit une clef de 168 bits. Cependant cette complexité est calculée basée sur la connaissance par l'attaquant d'un faible volume de correspondance de \textit{ciphertext} et \textit{plaintext} ($<2^{40}$). L'ANSSI recommande une solidité de clef supérieure à \solidityrecosizeAnssi.
...
\begin{Define}{Cryptographie!Réseau de Feistel}
...
\subsubsection{Chiffrement par S-Box et P-Box}
