...
\textbf{En Java}, Afin d’utiliser les expressions régulières en Java,  il faut utiliser les classes Pattern et Matcher appartenant au package java.util.regex.

\textbf{La classe Pattern :}

La première étape est de définir le motif que l'on veut utiliser et créer une instance de la classe Pattern avec ce motif. 
La classe Pattern dispose pas de constructeur, on obtient une instance de la classe grâce à la méthode compile appartenant à la classe Pattern qui prend comme seul et unique paramètre le motif sous forme d'un objet String.

\begin{Config}{Java}{JEE: Filtrage en entrée - REGEX}
Pattern pattern = Pattern.compile ("^\\w+@[a-z]+\\.[a-z]{2,4}$");
\end{Config}

\textbf{La classe Matcher :}
...
\begin{Config}{Java}{JEE: Filtrage en entrée - REGEX}
Matcher matcher = pattern.matcher ("toto@gmail.com");
\end{Config}

\textbf{En PHP}, la librairie PCRE assure la gestion des \gls{REGEX}.\MarginPar{\textbf{PHP}: Filtrage en entrée - REGEX}{0cm} Cette librairie est présente nativement depuis PHP 4.0.
...
\subsubsection{Java: Utilisation de Prepared Statement}
...
\subsubsection{PHP: Utilisation de l'API Php Data Object (PDO)}

De même qu’en Java, il existe en PHP la classe \textbf{PDOStatement} permet de créer et d’exécuter des instructions SQL précompilées.
...
Prototype des deux méthodes:

\begin{Config}{php}{PHP: Méthodes prepare et execute}
public PDOStatement PDO::prepare ( string $statement [, array $driver_options = array() ] )
public bool PDOStatement::execute ([ array $input_parameters ] )
\end{Config}

Une requête SQL crée par la méthode \textit{prepare} peut contenir des paramètres nommés ( :name) ou des points d’interrogation qui seront ensuite remplacés par les paramètres désirés à l’exécution de la requête.
\textbf{Il est important de lier ces paramètres aux entrées utilisateur et de ne pas utiliser les entrées utilisateur directement dans la requête}, ce qui rendrait l’utilisation des instructions précompilées inutile.

Requête SQL avec paramètres nommés:
\begin{Config}{php}{PHP: Création d'une requête précompilée}
<?php
...
Requête SQL avec points d’interrogations :
\begin{Config}{php}{PHP: Création création d'une requête précompilée}
<?php
/* Execute a prepared statement by passing an array of values */
