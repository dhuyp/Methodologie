...
L'attaque de moindre complexité pour AES consiste en une attaque par brute-force, la difficulté de l'attaque est donc fonction de la taille de la clef avec une complexité cryptographique de 128 bits pour AES-128 et 256 bits pour AES-256. La complexité cryptographique de DES est de 80 pour 2DES (ou \textit{2TDEA}) soit une clef de 112 bits et 112 pour 3DES soit une clef de 168 bits. Cependant cette complexité est calculée basée sur la connaissance par l'attaquant d'un faible volume de correspondance de \textit{ciphertext} et \textit{plaintext} ($<2^{40}$). L'ANSSI recommande une solidité de clef supérieure à \solidityrecosizeAnssi bits.
...
\end{array}
\right.$\\
Le déchiffrement s'effectue alors par (en fonction,\\
$\left\{
\begin{array}{l}
...
\end{Define}
Lors du premier \textit{round}, la round key est XORé octet par octet avec le \textit{state}, tel que $a_{i,j}\oplus k_{i,j}=b_{i,j}$ où b est le nouvel état interne. Lors des étapes intermédiaires, le state subit les étapes suivantes\footnote{Ces étapes sont:\\\textcircled{1}SubBytes: Utilisation de la S(ubstitution)-Box\\\textcircled{2}ShiftRows: Permutation sur la ligne\\\textcircled{3}MixColumns: Utilisation d'une transformation par une matrice fixée\\\textcircled{4}AddRoundKey: XOR avec l'octet correspondant de la sous-clef} (considérant a comme l'état intermédiaire actuel, b comme l'état suivant et i et j comme les coordonnées au sein de la matrice d'état):\\
$\left\{
\begin{array}{l r}
\forall i,j \in [[0,3]], a_{i,j}\rightarrow_{S-Box} b_{i,j} &\textcircled{1}\\
\forall i,j \in [[0,3]], a_{i,j}\rightarrow  b_{i- j\pmod{4},j} &\textcircled{2}\\
\forall j \in [[0,3]],
\begin{bmatrix}
2&3&1&1\\
1&2&3&1\\
...
b_{3,j}
\end{bmatrix}
in GF_{2^8} &\textcircled{3}\\
\forall i,j \in [[0,3]], a_{i,j}\oplus k_{i,j}=b_{i,j} &\textcircled{4}
\end{array}
\right.$\\
L'étape finale n'applique que 1,2 et 4.
